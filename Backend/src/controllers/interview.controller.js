...
async function deleteInterviewReportController(req, res) {
    try {
        ...
        const deleted = await interviewReportModel.findOneAndDelete({
            _id: interviewId,
            user: req.user.id  // sirf apna report delete kar sake
        })
        ...
        console.log(error)
        res.status(500).json({ message: "Failed to delete interview report.", error: error.message })
    }
}
